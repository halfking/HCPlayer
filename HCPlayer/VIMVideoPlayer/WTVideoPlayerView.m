...
- (void)checkPause
{
    ...
    if(secondsPlaying_ >= secondsDuration_) //???
    {
        ...
        {
            self.playing = NO;
            if(self.delegate && [self.delegate respondsToSelector:@selector(videoPlayerView:pausedByUnexpected:item:)])
            {
                __weak WTVideoPlayerView * weakSelf = self;
                __weak AVPlayerItem * weakItem = playerItem_;
                dispatch_async(dispatch_get_main_queue(), ^(void)
                               {
                                   [self.delegate videoPlayerView:weakSelf pausedByUnexpected:nil item:weakItem];
                               });
...
-(void)avPlayerPlaybackStalled:(NSNotification *)notification
{
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(videoPlayerView:didStalled:)]){
        [self.delegate  videoPlayerView:self didStalled:notification.object ];
    }
...
- (void)avPlayerItemFailedToPlayToEnd:(NSNotification *)notification
{
    ...
    {
        [self.delegate videoPlayerView:self didFailedToPlayToEnd:[notification.userInfo objectForKey:AVPlayerItemFailedToPlayToEndTimeErrorKey]];
    }
}
...
{
    if(self.delegate && [self.delegate respondsToSelector:@selector(videoPlayerView:didTapped:)])
    {
        [self.delegate videoPlayerView:self didTapped:nil];
    }
}
#pragma mark - videoPlayerDelegate
- (void)videoPlayerIsReadyToPlayVideo:(WTVideoPlayerView *)videoPlayer
{
    [self setDurationWithPlayeritem:videoPlayer.playerItem];
    
    if ([videoPlayer.delegate respondsToSelector:@selector(videoPlayerViewIsReadyToPlayVideo:)])
    {
        [videoPlayer.delegate videoPlayerViewIsReadyToPlayVideo:videoPlayer];
    }
}

- (void)videoPlayerDidReachEnd:(WTVideoPlayerView *)videoPlayer
{
    if ([videoPlayer.delegate respondsToSelector:@selector(videoPlayerViewDidReachEnd:)])
    {
        [videoPlayer.delegate videoPlayerViewDidReachEnd:videoPlayer];
    }
}

- (void)videoPlayer:(WTVideoPlayerView *)videoPlayer timeDidChange:(CGFloat)secondsPlaying
{
    [self hideActivityView];
    //显示歌词
    ...
    {
        [videoPlayer.delegate videoPlayerView:videoPlayer timeDidChange:secondsPlaying];
    }
}

- (void)videoPlayer:(WTVideoPlayerView *)videoPlayer loadedTimeRangeDidChange:(float)duration
{
    if ([videoPlayer.delegate respondsToSelector:@selector(videoPlayerView:loadedTimeRangeDidChange:)])
    {
        [videoPlayer.delegate videoPlayerView:videoPlayer loadedTimeRangeDidChange:duration];
    }
}

- (void)videoPlayer:(WTVideoPlayerView *)videoPlayer didFailWithError:(NSError *)error
{
    if ([videoPlayer.delegate respondsToSelector:@selector(videoPlayerView:didFailWithError:)])
    {
