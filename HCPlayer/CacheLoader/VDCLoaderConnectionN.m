...
@interface VDCLoaderConnectionN ()<VDCManagerRequestTaskNDelegate>
...
@implementation VDCLoaderConnectionN
...
- (void)dealWithLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest
{
    ...
    if (!self.task) {
        self.task = [[VDCTempFileManagerN alloc] init];
...
- (void)task:(VDCTempFileManagerN *)task didReceiveVideoLength:(long long)videoLength mimeType:(NSString *)mimeType
{
    
}

- (void)didReceiveVideoDataWithTask:(VDCTempFileManagerN *)task
{
    [self processPendingRequests];
